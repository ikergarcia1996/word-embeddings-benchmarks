...
def evaluate_on_semeval_2012_2(w):
    ...
    for c in categories:
        ...
        prot_left = np.mean(np.asarray([w.get(word, mean_vector) for word in prototypes[:, 0]]), axis=0)
        prot_right = np.mean(np.asarray([w.get(word, mean_vector) for word in prototypes[:, 1]]), axis=0)

        questions = data.X[c]
        question_left, question_right = np.asarray([w.get(word, mean_vector) for word in questions[:, 0]]), \
                                        np.asarray([w.get(word, mean_vector) for word in questions[:, 1]])
