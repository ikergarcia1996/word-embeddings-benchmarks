...
class SimpleAnalogySolver(sklearn.base.BaseEstimator):
    ...
    def predict(self, X):
        ...
        for id_batch, batch in enumerate(batched(range(len(X)), self.batch_size)):
            ...
            A, B, C = np.asarray([w.get(word, mean_vector) for word in X_b[:, 0]]), \
                      np.asarray([w.get(word, mean_vector) for word in X_b[:, 1]]), \
                      np.asarray([w.get(word, mean_vector) for word in X_b[:, 2]])
